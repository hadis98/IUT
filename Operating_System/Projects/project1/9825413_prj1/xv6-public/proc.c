...
int pssyscall()
{
  ...
  acquire(&ptable.lock);
  cprintf("name \t pid \t state \t     parent \t\t siblings \n");
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    int siblings[NPROC];
    int counter = 0;
    ...
    {
      if ((p->pid != p2->pid) && (p2->parent->pid == p->parent->pid) && (counter == 0 || (counter != 0 && siblings[counter - 1] != p2->pid)))
      {
        if ((p->state == SLEEPING || p->state == RUNNING || p->state == RUNNABLE) &&
            (p2->state == SLEEPING || p2->state == RUNNING || p2->state == RUNNABLE)) //
        {          
          siblings[counter] = p2->pid;
          counter++;
        }
      }
    }
    int siblings_num = counter;
    if (p->state == SLEEPING)
    {
