...
static const yytype_int16 yyrline[] =
{
     ...
     237,   239,   246,   247,   249,   250,   249,   261,   260,   269,
     271,   272,   274,   275,   277,   295,   318,   341,   361,   381,
     401,   402,   403,   404,   405,   405,   406,   406,   407,   410,
     444,   447,   452,   460,   469,   479,   492,   503,   513,   522,
     530,   536,   547,   548,   550,   551,   553,   571,   591,   598,
     606,   615,   623,   631,   660,   689,   718,   746,   795,   842,
     872,   902,   939,   961,   993,  1055,  1115,  1136,  1164,  1178,
    1179,  1191,  1178,  1196,  1204,  1206,  1208,  1212,  1206,  1244,
    1249,  1256,  1256
};
#endif
...
#line 1550 "parser.tab.c"
  ...
  case 21: /* block: TOKEN_LCB declare_vars statments TOKEN_RCB  */
#line 240 "parser.y"
                {//CG
...
#line 278 "parser.y"
			...
			if(scalar_symbol->symbol_type != ((yyvsp[-1].Expr))->expr_type) yyerror("Operands of = Operation should have same type");
			//*Code Generation	
            string Rs=Stack.top();
            Stack.pop();
            MIPS_FILE = fopen("MIPS.asm", "a+");
            if(Rs[0]!='$')
            	fprintf(MIPS_FILE,"\taddi %s,$zero,%s\n", (getSymbol((yyvsp[-3].str_val),currentScope)->reg).c_str(), Rs.c_str());
            else{
...
#line 296 "parser.y"
        ...
        if(Rs[0]!='$'){
        	...
        	string RRs = (getSymbol((yyvsp[-3].str_val),currentScope)->reg).c_str();
...
#line 319 "parser.y"
        ...
        if(Rs[0]!='$'){
        	...
        	string TTs = (getSymbol((yyvsp[-3].str_val),currentScope)->reg).c_str();
...
#line 411 "parser.y"
				...
				if(func->num_args != ((yyvsp[-1].Args))->num_args) yyerror("number of used arguments does not match with defined function!!");
         ...
         if(func==NULL) yyerror(strdup("undefined function"));
           ...
           if(func->num_args != ((yyvsp[-1].Args))->num_args) yyerror(strdup("the number of arguments does not match"));
            ...
            //*Code Generation
             MIPS_FILE = fopen("MIPS.asm", "a+");
             string arguments[4];
             for(int i=((yyvsp[-1].Args))->num_args-1 ; i>=0 ; i--){
               arguments[i] = Stack.top();
               Stack.pop();
             }
             for(int i=0; i<((yyvsp[-1].Args))->num_args; i++){
   				if(arguments[i][0]!='$')
...
#line 599 "parser.y"
		...
		(yyval.Expr) = pnt;	
		Stack.push(convertToString((yyvsp[0].int_val)));		
	}
...
#line 903 "parser.y"
        ...
        Stack.pop();
        string Rd = getNewRegister('t');
         if(((yyvsp[0].Expr))->expr_val_int == 0 || Rs[1]=='s' || Rt[1]=='s') cout<<"divide by zero on line "<<numLines<<endl;
  		if(Rs[0]!='$' && Rt[0]!='$')
  			fprintf(MIPS_FILE,"\taddi %s,$zero,%d\n", Rd.c_str(),convertStringToInt(Rs)/convertStringToInt(Rt));
  		else if(Rs[0]!='$'){
...
#line 1116 "parser.y"
        ...
        Stack.pop();
        if(Rs[0]!='$')
          Stack.push("-"+convertToString((yyvsp[0].Expr)-> expr_val_int));
  		else{
  		  	string Rd = getNewRegister('t');
  		  	MIPS_FILE = fopen("MIPS.asm", "a+");
...
#line 2694 "parser.tab.c"
  ...
  case 93: /* optional_else: TOKEN_ELSECONDITION block  */
#line 1197 "parser.y"
                {
          Label++;
          MIPS_FILE = fopen("MIPS.asm", "a+");
...
#line 1212 "parser.y"
		...
		//!RISK
		pushSymbol((yyvsp[-5].str_val),currentScope,getNewRegister('s'),"int",false,0);	
		string Rt = Stack.top();		
         Stack.pop();
         string Rs = Stack.top();		 
         Stack.pop();
         MIPS_FILE = fopen("MIPS.asm", "a+"); 
		fprintf(MIPS_FILE,"\taddi %s,$zero,%s\n", (getSymbol((yyvsp[-5].str_val),currentScope)->reg).c_str(), Rs.c_str());		
		 cout<<"what?\n";
		 //!CHECK
		// if(Rs[0]!= '$'){
        ...
        // }
		 ...
		 cout<<"\nRd\n";
         fprintf(MIPS_FILE,"\taddi %s,$zero,%s\n", Rd.c_str(), Rt.c_str());
         fprintf(MIPS_FILE,"\tslt %s,%s,%s\n", Rd.c_str(), Rd.c_str(), (getSymbol((yyvsp[-5].str_val),currentScope)->reg).c_str());		
         fprintf(MIPS_FILE,"\txori %s,%s,1\n", Rd.c_str(), Rd.c_str());
         Label++;
   		fprintf(MIPS_FILE,"\tbeq %s,$zero,L%d\n", Rd.c_str(), Label);
         fclose(MIPS_FILE);
